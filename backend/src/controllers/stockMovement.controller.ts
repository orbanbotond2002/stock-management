...
export default async function stockMovementController(
      ...
      schema: {
        body: {
          ...
          properties: {
            ...
            sourceWarehouseId: {
              type: 'string',
              description: 'Required for OUT and TRANSFER',
            },
            targetWarehouseId: {
              type: 'string',
              description: 'Required for IN and TRANSFER',
            },
          },
        },
      },
        ...
        switch (body.type) {
          case MovementType.IN:
            if (!body.targetWarehouseId)
              throw validationError(
                'targetWarehouseId is required for IN movement'
              );
          ...
          case MovementType.OUT:
            if (!body.sourceWarehouseId)
              throw validationError(
                'sourceWarehouseId is required for OUT movement'
              );
          ...
          case MovementType.TRANSFER:
            if (!body.sourceWarehouseId || !body.targetWarehouseId) {
              throw validationError(
                'Both sourceWarehouseId and targetWarehouseId are required for TRANSFER movement'
              );
            }
